...
public record JournalView(
    ...
    public record StatLine(String name, String abbr, int level, int xp) {}
    ...
    public static JournalView from(Player player, int days) {
        ...
        for (StatType type : StatType.values()) {
            stats.add(new StatLine(type.getDisplayName(), type.getAbbreviation(),
                    s.getLevel(type), s.getCurrentXP(type)));
        }
