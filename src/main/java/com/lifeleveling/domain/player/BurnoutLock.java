 ...
 *   - Penalización Económica: -10% del Gold total (aplicada externamente)
 *   - Condición de Salida: Timer expirado + HP > 0
 * Filosofía:
 * El Burnout no es un "Game Over", es un "Modo Supervivencia" que obliga
 * al jugador a priorizar autocuidado (dormir, comer) sobre productividad.
 */
...
public record BurnoutLock(
    ...
    public BurnoutLock {
        if (id == null) {
            throw new IllegalArgumentException("El ID del BurnoutLock no puede ser null");
        }
        if (triggeredAt == null) {
            throw new IllegalArgumentException("El timestamp de trigger no puede ser null");
        }
        if (expiresAt == null) {
        ...
        if (expiresAt.isBefore(triggeredAt)) {
            throw new IllegalArgumentException(
                    String.format("La fecha de expiración (%s) no puede ser anterior al trigger (%s)",
        ...
        if (actualDuration.compareTo(LOCKOUT_DURATION) < 0) {
            throw new IllegalArgumentException(
                    String.format("La duración del lock (%s) debe ser al menos %s",
                            actualDuration, LOCKOUT_DURATION)
            );
        }
    }
    ...
    public Duration getTimeElapsed(Instant currentTime) {
        ...
        if (currentTime.isBefore(triggeredAt)) {
            return Duration.ZERO;
        }
    ...
    public String formatTimeRemaining(Instant currentTime) {
        ...
        if (hours > 0) {
            ...
            return String.format("%dm restantes", minutes);
        }
    }

    public String toDisplayString(Instant currentTime) {
        if (hasExpired(currentTime)) {
            return "✅ BURNOUT SUPERADO";
        }
    ...
    public String toString() {
        return String.format("BurnoutLock[id=%s, triggered=%s, expires=%s, duration=%s]",
                id, triggeredAt, expiresAt, LOCKOUT_DURATION);
    }
}